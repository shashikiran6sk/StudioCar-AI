...
// Local storage endpoint; intercept them even when a local bucket override is set.
...
inventoryTest(
  ...
  async ({ page }, testInfo) => {
    ...
    try {
      ...
      await expect(vehicleDialog.getByRole("textbox", { name: "Model" })).toBeVisible();
      await expect(vehicleDialog.getByRole("textbox", { name: "Internal ID" })).toBeVisible();
      await expect(vehicleDialog.getByText("Add vehicle specifications (optional)"))
        .toHaveCount(0);
      await page.screenshot({
        animations: "disabled",
        path: testInfo.outputPath("restored-vehicle-details.png"),
      });
