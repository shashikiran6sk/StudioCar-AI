...
- Re-process, Replace and new studio versions exist since SC043 and all submit through the ordinary processing command. Favorites, edit, hero selection, and ZIP actions remain intentionally absent until their canonical persistence, mutation, and archive-generation flows exist; the UI does not expose controls that would falsely imply those operations are implemented.
