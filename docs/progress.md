...
### SC043 — Selection Dialog as the central workflow: new studio versions, attention, Re-process, Replace

Before this slice a vehicle could be processed exactly once: the batch reservation accepted only a `DRAFT` vehicle, the portfolio showed only the newest completed batch and returned 404 otherwise, and the worker marked a vehicle `PARTIALLY_FAILED` from the failures of every batch it had ever run. The schema already allowed many batches per vehicle (jobs are keyed by `batchIdempotencyKey`), so no migration was needed.

- **Quick actions.** Upload a vehicle and View inventory are unchanged. Recent results is gone — it duplicated Inventory and the Recent vehicles section. The third card is now dynamic: **Create studio images** (opens `/inventory?mode=CREATE_STUDIO`) when nothing needs the user, or **Attention needed** with the number of affected **vehicles**. One affected vehicle opens its portfolio at `#attention`; several open `/inventory?filter=NEEDS_ATTENTION`. Vehicles are counted rather than images so three failed photos of one car read as one thing to fix; the portfolio shows the real failed-image count.
- **One attention rule.** `jobRequiresUserAttention` in `packages/processing` (`FAILED` or `CANCELLED`) is the job-level rule. `FAILED` is written only once automatic retries are exhausted or the failure cannot be retried, so `RETRYING` never asks for attention. The worker applies that rule atomically to the vehicle's **newest batch only** and records `PARTIALLY_FAILED`; `NEEDS_ATTENTION_VEHICLE_STATUSES` is the vehicle-level list the dashboard count, the Inventory filter and the portfolio panel all read.
- **Worker fix.** A vehicle's aggregate status is now decided by the batch that just finished, not by every failure it ever had, so a successful re-process clears attention. Only one batch runs per vehicle, so once nothing is active the newest batch is the one that finished.
- **Reservations for existing vehicles.** A batch may now start for a `DRAFT`, `READY` or `PARTIALLY_FAILED` vehicle. A `PROCESSING` or `ARCHIVED` vehicle is refused as `VEHICLE_UNAVAILABLE` with a message the dialog shows. The conditional claim means differently keyed submissions racing for one vehicle create exactly one batch; the dialog keeps one idempotency key per session and a ref guard blocks same-frame double clicks. A new batch creates its own jobs and never touches earlier ones, so completed results and failed history are preserved, and no duplicate vehicle is created for a background change.
- **Batch identity.** Inventory and the portfolio now group a batch by its idempotency key instead of its request hash: an unchanged re-process has the same hash as the batch it retries and must still count as its own batch.
- **Studio versions.** The portfolio groups completed images into versions by treatment (`createProcessingOptionsKey`, a SHA-256 of canonical options), holding the newest completed image per original. A same-treatment retry of only the failed photos therefore completes its version rather than hiding the photos that already succeeded, and a different background or floor becomes a separate version. `?version=` selects which version the gallery shows; the portfolio also lists every version, shows a processing notice while a new batch runs, and renders for vehicles with no completed output yet.
- **Portfolio failure UI.** The attention panel names each failed image (position, filename, original thumbnail), the treatment used, and a user-facing reason from `ProcessingFailureReasonSchema` (`UNUSABLE_IMAGE`, `BACKGROUND_REMOVAL_FAILED`, `SERVICE_UNAVAILABLE`, `PROCESSING_FAILED`, `CANCELLED`). Provider codes and worker messages never reach the contract. Arriving from Review issues focuses the panel. Re-process and Replace link to `?studio=REPROCESS_FAILED` / `?studio=REPLACE_FAILED`.
- **One Selection Dialog, four modes.** `VehicleCreateDialog` accepts a server-built `StudioSelectionContext` (`CREATE_VARIANT`, `REPROCESS_FAILED`, `REPLACE_FAILED`; `NEW_UPLOAD` is the unchanged trigger flow). An existing vehicle skips the details step, so the stepper counts three steps. Photos from earlier batches appear as selectable rows; failed rows show the reason and a Replace image action; an original the worker marked `INVALID` cannot be selected and must be replaced. Re-process preselects only failed, usable photos so successful images are not re-processed unless chosen. A replacement is uploaded straight to S3 for the same vehicle, takes the failed photo's place, and the failed original stays stored for history.
- **Navigation.** The dialog is URL-driven. From create-studio Inventory, Cancel returns to `/inventory?mode=CREATE_STUDIO` and Process continues to Inventory. From a portfolio, Cancel and Process return to that portfolio. Both exits replace the URL, so Back never reopens a finished dialog. Every mode submits through the same `POST /api/jobs` command, outbox, queue and worker, registers the jobs with the existing status poller, and refreshes the server-rendered page.
- **Inventory.** The Failed filter is now **Needs attention** (`NEEDS_ATTENTION`). Create-studio mode lists only `READY` and `PARTIALLY_FAILED` vehicles and gives each card a Create images action. Cards expose `hasCompletedOutput`, so a vehicle getting a new version keeps an Open portfolio link while processing, and a failed vehicle offers Review issues.
- **Stale attention after a replacement (found in local testing).** A vehicle whose replacement batch completed still showed Attention needed. The local image worker was running an image built before this slice, which still counted failures from every batch, so it recorded `PARTIALLY_FAILED`. Rebuilding with `pnpm infra:build` loads the fixed rule, but the vehicle stayed wrong because nothing recomputes a finished vehicle. Migration `20260923180000_reconcile_vehicle_attention` moves exactly those vehicles — `PARTIALLY_FAILED` with a newest batch in which every job completed — to `READY`. It is idempotent, leaves real failures, retries and job-less vehicles alone, and has a real-PostgreSQL test.
...
### SC046 — Plan catalog rename, homepage account menu, portfolio ZIP, quick-action imagery, scrollable Selection Dialog

- **Studio Plus retired; Studio Pack is now Studio Plus.** The catalog ships Free, Studio Plus (₹1,499 one-time, 100 images, 20 per batch, lifetime allowance — the former Studio Pack) and Studio Pro, unchanged. `PlanKeySchema` is `FREE`, `STUDIO_PLUS`, `STUDIO_PRO`; `STUDIO_PACK` no longer exists anywhere in code or tests.
- Migration `20260923220000_retire_studio_plus_rename_studio_pack` moves the retired plan out of the way before the key is reused: its subscriptions are expired and relabelled `RETIRED_STUDIO_PLUS` (which the application already resolves to Free), its `PlanConfig` row is deleted, and then the `STUDIO_PACK` rows are renamed. A display name an administrator had already changed is kept. A rolled-back real-PostgreSQL test proves both paths without disturbing the shared plan rows other integration files read.
- The homepage no longer shows a separate Dashboard button to a signed-in visitor. The account menu gains a Dashboard link beside Profile & security and Log out (`AccountMenu showDashboardLink`); the workspace menu is unchanged, since its sidebar already leads there.
- **Portfolio ZIP download.** "Download all (ZIP)" saves every image of the studio version on show. The browser fetches each processed image through its existing short-lived signed attachment URL and assembles a store-only ZIP with `fflate`, so no image bytes pass through Next.js. Entries use the same `processed-NN.ext` names as single downloads (now carried as `downloadFilename` in the portfolio contract) and never overwrite one another. Progress counts images actually received; any failed fetch, such as an expired link, saves nothing and explains how to recover.
- The upload bucket's CORS rule now allows `GET` as well as `PUT`, which the in-browser ZIP needs. **Existing AWS stacks must be updated** before the button works there.
- **Quick-action imagery.** Each action now has a picture of what it does rather than the same sedan: photos going into an upload area, a grid of inventory cards, one photo split into street-before and studio-after, and a batch with one flagged image for Attention needed (a new, fourth asset). They are compositions of the licensed `silver-sedan.png` with generated grounds, so provenance is unchanged; the four WebP files total 67 KB.
- **Selection Dialog footer reachable.** With more than a handful of photos the dialog's body grew past the viewport and its Cancel/Next bar was clipped with no way to scroll. `.sc-dialog__content` is now a flex column whose body alone shrinks and scrolls, and each wizard step's action bar is sticky at the bottom of that body.
- Added unit coverage for the ZIP helpers, button states and gallery, the account menu link, the attention image, and the renamed catalog; Playwright now downloads and unzips a portfolio ZIP, asserts the dialog actions stay in the viewport of a 420px-tall window, and opens the homepage account menu to reach the dashboard.

### SC047 — Explicit Local, Development, and Production environment profiles

- Added a required `APP_ENV` (`local`, `development`, `production`) and one authoritative profile table, `packages/config/src/environment-profiles.ts`, that decides the Google and phone sign-in drivers, email driver, background-removal provider, storage and queue targets, and worker runtime for each environment, plus the explicit overrides each tolerates. `NODE_ENV` keeps its Node.js meaning and no longer selects anything: every former `NODE_ENV === "production"` guard is now a profile rule.
- Every focused runtime parser keeps its secret isolation and now lays the profile's defaults under the configured values before validating. Only repository-owned infrastructure is defaulted, from one module (`local-infrastructure.ts`); external credentials, the Development session secret, and anything a deployment owns never are. The local queue connection is applied all or nothing, so a Development configuration that moves to AWS SQS cannot inherit the ElasticMQ endpoint or emulator keys.
- Local needs only `APP_ENV=local` and `REMOVEBG_API_KEY`. Added a `fake` Google driver behind the existing `GoogleIdentityProvider` port: it returns the browser to the application's own callback with a code bound to the one-time challenge's PKCE verifier, then resolves the fixed identity `developer@studiocar.local` through the ordinary challenge, identity, account, and session path. The existing fake phone driver (code `1234`) is now allowed only in Local.
- Development requires real Google OAuth, the real MSG91 Widget (all three credentials), its own database and session secret, and an AWS S3 bucket, while queues, both workers, the dispatcher, and Mailpit run locally. A missing Google or MSG91 value is a configuration error, never a fallback; the phone widget endpoint now fails instead of silently hiding phone sign-in.
- Production fails closed on fake sign-in, MinIO or localhost storage, ElasticMQ or non-AWS queue URLs, Mailpit, the local queue consumers, the local bucket, a bucket without a `prod`/`production` segment, any bucket/queue/database declaring a non-production environment, a local database, every committed local value, and a non-https redirect URI. Development refuses a production-declared bucket, queue, or database, and the committed session secret.
- Fixed Local uploads, which the content security policy blocked: it allowed only `https://*.amazonaws.com`, so a browser PUT to MinIO could never succeed. The Local profile's MinIO origin is now added to `img-src` and `connect-src`, and nothing is added for any other environment.
- Fixed Local and Development completion email, which could never be published: the worker contract validated `portfolioUrl` with `z.httpUrl()`, which demands a public hostname, while those environments build the link from `http://localhost:3000`. Production's https public origin remains enforced where `APPLICATION_BASE_URL` is validated.
- Replaced `apps/web/.env.example` with `.env.example.local`, `.env.example.development`, and `.env.example.production`, and made the repository-root `.env.local` the one settings file for the application, Prisma, `db:reset`, `db:seed`, and compose. Settings files left in `apps/web` are refused, and `.dockerignore` now keeps every `.env` file out of images (previously `COPY . .` copied them in).
- `pnpm infra:up` follows `APP_ENV`: Local starts the full stack; Development starts only ElasticMQ, Mailpit, both workers, and the dispatcher, with the workers pointed at the Development database and bucket. `pnpm app:up` is Local only, and every script refuses production. `pnpm db:reset` now also requires `APP_ENV=local` and reads the canonical local constants directly.
- The Lambda templates set `APP_ENV=production`; the storage template records the owning environment as a `studiocar:environment` tag and documents the bucket naming rule. The CI browser job runs the Local profile and no longer needs AWS test credentials.
- Added profile, default, resolution, isolation-rule, driver-selection, per-environment matrix, compose-consistency, and example-synchronization tests, plus fake Google provider (including a full start-to-session completion and one-time replay refusal), provider selection, settings-file loading, local storage origin, CSP, and email-contract tests. Added `docs/environments.md`.
...
### SC059 — Revert simplified new-vehicle metadata
...
- Portfolio URLs are signed only for the selected studio version's images, the newest batch's failed originals, and Selection Dialog previews, all chosen from the tenant-scoped repository's bounded job history. Originals use inline signed responses, processed outputs have separate inline and attachment signatures, and all URLs expire according to the bounded presigned URL configuration.
- Re-process, Replace and new studio versions exist since SC043 and all submit through the ordinary processing command. Since SC046 the selected version's images download as a ZIP assembled in the browser from their signed URLs. Favorites, edit, and hero selection remain intentionally absent until their canonical persistence and mutation flows exist; the UI does not expose controls that would falsely imply those operations are implemented.
- Attention is decided in one place: `jobRequiresUserAttention` for jobs, applied by the worker to the newest batch, and `NEEDS_ATTENTION_VEHICLE_STATUSES` for vehicles. Do not add status checks elsewhere.
- The browser suite runs under `APP_ENV=local`, so signed links address the Local profile's MinIO bucket and are intercepted before any request leaves; CI needs no AWS credential at all.
...
- Turborepo's E2E task passes only `APP_ENV` and `DATABASE_URL`; the Local profile supplies everything else, so authenticated Playwright tests execute without any secret reaching the browser-test task. The build task declares `APP_ENV` and `S3_ENDPOINT` and hashes the root `.env.local`, because the content security policy depends on them.
...
- Production credential ownership is defined in `docs/security.md`. `.env.example.production` is a reference contract annotated by runtime, never a file to deploy; focused runtime parsers and deployment configuration must prevent unrelated secrets from crossing process boundaries.
- Environment selection belongs to `APP_ENV` and `packages/config/src/environment-profiles.ts` only. Never branch on `NODE_ENV` or `APP_ENV` in feature, domain, or worker code; add a driver to the profile and select its adapter at a composition root.
