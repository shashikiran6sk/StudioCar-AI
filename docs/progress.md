...
### SC042 — Turntables that are actually drawn, from stored studio assets

- **Root cause of the missing turntable.** The customise step sent the floor, but the repository that stored a job's options listed fields by hand and left the floor out. The queue carries only a job ID, so the worker re-read the stored options, the schema defaulted the missing floor to the standard floor, and no turntable was ever drawn. None of the 18 jobs in the development database had a floor stored. The batch request hash had the same omission, so a retry that changed only the floor replayed the original batch. Both now copy every validated option, and regression tests fail if the floor is dropped at either point or ignored by the worker.
- **Semantic IDs.** A treatment is `backgroundId` plus `floorId`: three studios (Premium White, Dark Studio, Grey Studio), each with exactly two floors, a studio floor and a turntable. The options contract is a discriminated union, so a floor from another background does not parse and nothing is substituted. Dealership and Custom are removed from the contract, the dialog, the worker, the portfolio, and the homepage copy. A data migration rewrites stored options: earlier studio jobs, which were all drawn on the standard floor, record that floor, and the one development Dealership row is recorded as Premium White.
- **Selection Dialog.** Rebuilt from `docs/screens/Selection_Dialog/`: no background is preselected, floors appear once one is chosen with the first floor selected, and changing background never keeps the previous background's floor. Previews are bundled under `apps/web/public/studio-assets/`; opening and using the dialog makes no object-storage request, which was confirmed in the running application.
- **Worker assets from S3.** The procedurally drawn SVG scene is replaced by processing assets stored under `studio-assets/v1/` in the configured bucket, resolved from the two IDs, read through the existing storage port, and cached per process. They are loaded before remove.bg is called, so a missing asset fails retryably without a provider charge. The worker role can read only `studio-assets/*` in addition to tenant objects. `pnpm studio-assets:sync` publishes them idempotently and refuses to overwrite a different object under a versioned key.
- **Production-grade assets.** The supplied floor PNGs were mockups: the studio floors met the wall at a hard seam, two had a CGI grid, and the turntables were flat ellipses floating on the wall with no floor around them. A committed, deterministic generator draws 3840×2160 replacements to the same geometry the compositor reads: a seamless studio cove, and a turntable with a rim, a groove and its own cast shadow standing on a floor. The previews are rendered from the same files.
- **Placement.** The real outputs confirmed the standard floor sat too low. The vehicle kept the photo's framing, so a tightly cropped photo left almost no floor under the tyres, and its tyre line was measured from the provider's soft shadow rather than the tyres. The vehicle is now scaled to the floor's stand and its tyre line, the lowest row of solid pixels, is placed on the floor's contact line, or on the turntable's surface and centred on it. Maintain Composition keeps the photo's aspect ratio, growing the frame upward rather than cropping the platform. The vehicle is never enlarged. Enhancement applies to the vehicle only, so studio tones stay identical across a batch.
- **A bright line at the horizon**, found in the real outputs: sharp's raw output of a resized transparent layer mis-scales soft edges. Intermediate layers stay lossless PNG, and a test pins it.
- **Verification.** Seven real photos from the test set (a white SUV front and rear three-quarter views, an MPV in portrait, a red hatchback front and rear) were cut out by the real remove.bg API and composed in all six combinations. Two of them then went through the running application end to end: browser-equivalent upload to S3, `/api/jobs`, dispatcher, SQS, the rebuilt local worker, and remove.bg, for all six combinations. Every job completed with both IDs stored, and every output was inspected. Lint, typecheck, unit, real-PostgreSQL integration, build, and Playwright suites pass.
- **Known limits.** A photo that crops the car's tyres at its bottom edge still shows that cut edge on the floor. Plate masking remains unimplemented, as before.
...
- The complete four-step wizard is mounted from the dashboard and submits only through the real authenticated processing command. Custom and Dealership backgrounds were removed in SC042; a custom background would need a private background-asset upload and ownership flow first.
...
- Studio processing assets are immutable under `studio-assets/v1/`, and workers cache them for the life of the process. Publish a visual change under a new version prefix with a registry change; never overwrite `v1` in a shared bucket. `pnpm studio-assets:sync` must run against a bucket before a worker that reads it is deployed.
- Keep intermediate transparent layers encoded as PNG when compositing with sharp. Its raw output of a resized RGBA layer mis-scales low-alpha pixels, which draws bright fringes where layers fade.
